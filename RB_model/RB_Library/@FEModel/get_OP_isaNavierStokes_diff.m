function [N_diff, R_diff] = get_OP_isaNavierStokes_diff(self, curr_cond, opts)
...
for i=1:n_elem
    ...
    for k=1:n_base % loop over the test functions
        for j=1:n_base % loop over the shape functions
            ...
            r_uu1 = (mue + mt).*(dbdx(j,:).*dbdx(k,:) + dbdy(j,:).*dbdy(k,:));
            ...
            % get the galerkin contrubution
            r_vv1 = (mue + mt).*(dbdx(j,:).*dbdx(k,:) + dbdy(j,:).*dbdy(k,:));
...
N_diff = sparse(k1_glob, j1_glob, v1_glob, numel(curr_cond), numel(curr_cond));
R_diff = N_diff*reshape(curr_cond,[],1);
