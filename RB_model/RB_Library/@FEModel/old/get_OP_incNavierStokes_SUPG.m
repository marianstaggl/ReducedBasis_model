function [N, Nres] = get_OP_incNavierStokes_SUPG(self, curr_cond, opts)
...
for i=1:n_elem
    ...
    uval = uvec(curr_elem); vval = vvec(curr_elem); pval = pvec(curr_elem);
    ...
    xres = -mue*(d2udx2 + d2udy2) + rho*(u.*dudx + v.*dudy) + dpdx;
    yres = -mue*(d2vdx2 + d2vdy2) + rho*(u.*dvdx + v.*dvdy) + dpdy;
    
    % calculate the support length of the element (this part may has to be reviewed)
    [tau_m, tau_c] = get_stabpar_wikipedia(order, rho, mue, [u; v], weight);
    tau_c = 0;
%     [tau_m, tau_c] = get_stabpar_redbkit(order, rho, mue, [u; v], inv_jac);
    ...
    for k=1:n_base
        ...
        for j=1:n_base
            ...
            g_uu1 = tau_c.*(dbdx(j,:).*dbdx(k,:));
            g_uv1 = tau_c.*(dbdy(j,:).*dbdx(k,:));
            
            % get the stabilization contribution (Part 1/Oseen Part)
            s_uu1 = tau_m.*(-mue*(d2bdx2(j,:) + d2bdy2(j,:)) + rho*(u.*dbdx(j,:) + v.*dbdy(j,:))).*t_m; % convective and diffusive part of the momentum residual 
            ...
            uu1_val(count) = sum(weight.*(s_uu1 + g_uu1)); uu2_val(count) = sum(weight.*(s_uu2));
            uv1_val(count) = sum(weight.*(g_uv1)); uv2_val(count) = sum(weight.*(s_uv2));
            up1_val(count) = sum(weight.*(s_up1));
            
            %% calculate the v block (v test functions)
            % get the grad div stabilization (continuity residual)
            g_vu1 = tau_c.*(dbdx(j,:).*dbdy(k,:));
            g_vv1 = tau_c.*(dbdy(j,:).*dbdy(k,:));
            
            % get the stabilization contribution (Part 1/ Oseen Part)
            s_vv1 = tau_m.*(-mue*(d2bdx2(j,:) + d2bdy2(j,:)) + rho*(u.*dbdx(j,:) + v.*dbdy(j,:))).*t_m; % convective and diffusive part of the momentum residual 
            ...
            % integrate the residuals function
            vu1_val(count) = sum(weight.*(g_vu1)); vu2_val(count) = sum(weight.*(s_vu2)); 
            vv1_val(count) = sum(weight.*(s_vv1 + g_vv1)); vv2_val(count) = sum(weight.*(s_vv2));
            vp1_val(count) = sum(weight.*(s_vp1));
            
            %% calculate the p block (p test functions)
            % get the stabilization contribution (Part 1/ Oseen Part)
            s_pu1 = tau_m.*(-mue*(d2bdx2(j,:) + d2bdy2(j,:)) + rho*(u.*dbdx(j,:) + v.*dbdy(j,:))).*dbdx(k,:); % 
            s_pv1 = tau_m.*(-mue*(d2bdx2(j,:) + d2bdy2(j,:)) + rho*(u.*dbdx(j,:) + v.*dbdy(j,:))).*dbdy(k,:); % 
