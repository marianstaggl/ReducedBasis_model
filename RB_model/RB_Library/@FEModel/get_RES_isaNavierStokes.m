...
r_au1 = (gamma-1)/2*(a.*dudx).*permute(b, [3, 1, 2]);
au1_val = reshape(sum(r_au1.*weights, 2), [], 1)';

r_av1 = (gamma-1)/2*(a.*dvdy).*permute(b, [3, 1, 2]);
av1_val = reshape(sum(r_av1.*weights, 2), [], 1)';

r_aa1 = (u.*dadx + v.*dady).*permute(b, [3, 1, 2]);
aa1_val = reshape(sum(r_aa1.*weights, 2), [], 1)';
