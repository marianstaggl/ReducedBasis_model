function [N1_conv, N2_conv, R_conv] = get_OP_isaNavierStokes_conv(self, curr_cond, opts)
...
for i=1:n_elem
    ...
    for k=1:n_base % loop over the test functions
        for j=1:n_base % loop over the shape functions
            ...
            r_uu1 = (u.*dbdx(j,:) + v.*dbdy(j,:)).*b(k,:); % convective part
            ...
            r_vv1 = (u.*dbdx(j,:) + v.*dbdy(j,:)).*b(k,:); %convective  part
...
N1_conv = sparse(k1_glob, j1_glob, v1_glob, numel(curr_cond), numel(curr_cond));
N2_conv = sparse(k2_glob, j2_glob, v2_glob, numel(curr_cond), numel(curr_cond));
R_conv = N1_conv * reshape(curr_cond,[],1);
end
