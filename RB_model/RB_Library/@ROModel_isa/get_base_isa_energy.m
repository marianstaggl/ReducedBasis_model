function base_fun = get_base_isa_energy(self, opts)
...
a_sel = self.fe_model.get_var_sel({'a'});
...
% decompose the primary and viscos fields of the boundary
...
% in order to perform an energy based decomposition scale the a values´
weight = sqrt(2*opts.alpha / (opts.kappa-1));
bound_prim(a_sel, ~bp_sel) = bound_prim(a_sel, ~bp_sel) * weight;
[domain_prim, dp_sel, dp_decay] = decompose_field(bound_prim(:, ~bp_sel), ...
    opts.primary_domain_th);
domain_prim(a_sel, :) = domain_prim(a_sel, :) / weight;
...
n_test = min([10*n_shape, size(domain_prim, 2)]); % allow a max of 4 for overdetermination
base_fun.prim_test = [mean_prim, bound_prim(:, bp_sel), ...
    domain_prim(:, 1:n_test)];
base_fun.prim_test(a_sel, :) = base_fun.prim_test(a_sel, :) * 2*opts.alpha / (opts.kappa-1);
base_fun.visc_shape = [mean_visc, bound_visc(:, bv_sel)];

% weight the test functions to generate energy based inner products
% base_fun.prim_test(a_sel, :) = base_fun.prim_test(a_sel, :) * weight^2;
